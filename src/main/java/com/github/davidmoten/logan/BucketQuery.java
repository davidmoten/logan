...
public class BucketQuery {
     ...
     * 
     */
    public BucketQuery(long startTime, double intervalSizeMs, long numIntervals,
            Optional<String> field, Optional<String> source, Optional<String> text,
            Optional<Integer> scan, Optional<String> delimiterPattern) {
        Preconditions.checkNotNull(source, "source must not be null but can be Optional.absent()");
        Preconditions.checkNotNull(text, "text must not be null but can be Optional.absent()");
        Preconditions.checkNotNull(startTime, "startTime must not be null");
        Preconditions.checkNotNull(field, "field must not be null but can be Optional.absent()");
        Preconditions.checkNotNull(scan, "scan must not be null but can be Optional.absent()");
        Preconditions.checkArgument(!(scan.isPresent() && field.isPresent()),
                "if scan is specified then field must not be specified");
        Preconditions.checkArgument(!(scan.isPresent() && scan.get() <= 0), "scan must be >0");
     ...
     *            field to filter on
     */
    public BucketQuery(Date startTime, double intervalSizeMs, long numIntervals, String field) {
        this(startTime.getTime(), intervalSizeMs, numIntervals, Optional.of(field),
     ...
     * 
     * @return true if and only if aggregation is requested for this query
     */
     ...
     * 
     * @return finish time for the query
     */
