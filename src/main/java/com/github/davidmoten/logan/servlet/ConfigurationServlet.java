...
public class ConfigurationServlet extends HttpServlet {
    ...
    private void sample(HttpServletRequest req, HttpServletResponse resp) {
        ...
        Configuration configuration = m.unmarshal(
                PropertyReplacer.replaceSystemProperties(new ByteArrayInputStream(xml.getBytes())));
        Data data = new DataMemory(configuration.maxSize);
        Util.addDummyData(data);
        FileTailerSampling sampler = FileTailerSampling.Singleton.INSTANCE.instance();
        ...
        for (Entry<LogFile, SampleResult> en : sampler.getSamples().entrySet()) {
            ...
            Set<Entry<LogEntry, List<String>>> items = en.getValue().getEntries().entrySet();
            for (Entry<LogEntry, List<String>> info : items) {
                ...
                out.println("</pre>");
                out.println("<p style=\"margin-left:50px;\">" + info.getKey() + "</p>");
            }
